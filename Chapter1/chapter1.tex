...
The ease of application of the two methods is varied. While TMC is conceptually easy to understand and incorporates many advantageous features, there are limitations in its application currently, such the failure of TALYS to correctly predict the behaviour of light nuclides (the typical cut-off being fluorine) \cite{Rochman2011}. For perturbation methods, the paucity of covariance information is a problem for some calculations (principally those outside the thermal fission realm). 
...
\label{subsec:data}
As discussed in section~\ref{subsubsec:tmc}, traditionally cross-section uncertainties are represented as single values for a given energy and reaction channel. There might be an estimate of uncertainty, which is typically given as a 1 $\sigma$, standard deviation of a Gaussian distribution centered about the quoted mean value. As an example, the $^{208}$Pb(n,2n)$^{207}$Pb neutron multiplication reaction at 14MeV is shown in Table \ref{tab:lead_by_lib} for a variety of experiments and nuclear data libraries. Experimental results from the EXFOR library are also shown. Most modern nuclear data libraries quote a figure of around 2.1b at 14 MeV. However, many experimental results, for example Arakita and Shunk are radically lower at 1.04 and 1.31 b respective and with estimated uncertainties of 5.77 and 8.0\% respectively. These values are among the older data experimental data and have been relegated in importance by nuclear evaluators. Consequently, most of the nuclear data libraries agree on a $\sigma_{n,2n}(14 MeV) \approx 2.1$b. The experimental data of table~\ref{tab:lead_by_lib} are also shown in figure~\ref{fig:tendl_lead}.
...
The distribution of the $r_{V}$ input parameter as sampled for the TBR simulations is shown as figure~\ref{fig:pb208_rvadjust}. It is not normally distributed, but rather positively skewed. It is unimodal. 
