...
That is, the rebar diameter, $d_{r}$ squared divided by the wall thickness, $t_{w}$ is proportional to the steel fraction, $f_{s}$. The value of 4.45\% was chosen as this is a typical value computed from the description of reinforcement and dimensions in \cite{Perez14}. 
...
% column span figure
\begin{figure}[H]
  \figuretitle{ICRP74 fluence to dose factors}
...
\begin{table}[H]
  ...
  %\begin{tabu} to \textwidth {X[3] X X X[5]}
    \toprule
    Activation product  & t$_{\frac{1}{2}}$(s)  & Decay mode   \\
    \midrule
    \textsuperscript{16}N & 7.13 & $\beta$               \\
    \textsuperscript{17}N & 4.14 & $\beta \rightarrow n$ \\
    \bottomrule
  \end{tabular}
  \caption[Decay paths for radioactive nitrogen.]{The parent, half-life and principal decay mode from activated nitrogen isotopes in the ITER water cooling system.}
  \label{tab:nitrogen_decay}
\end{table}

The nitrogen isotopes listed in table~\ref{tab:nitrogen_decay} decay through $\beta$ and neutron emission to oxygen isotopes, however they may still be in an excited state and so will emit gamma radiation until they achieve their ground state. The gamma decay of \textsuperscript{16}O is utilised as a source for several of the simulations presented in section~\ref{subsec:prompt}.
